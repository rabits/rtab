...
{
    Q_OBJECT
    Q_PROPERTY(quint8       firstFret READ firstFret WRITE firstFret NOTIFY firstFretChanged)
...
public:
    explicit CChord(quint8 length, QObject *parent = 0);
    CChord(const CChord &obj);
    CChord& operator=(const CChord &obj);

    // ---API--- //
    inline quint8              firstFret() const { return m_firstFret; }
    inline const QList<qint8>& strings()   const { return m_strings; }
    inline const QString&      name()      const { return m_name; }
    inline CBeat*              beat()      const { return p_beat; }

    inline void firstFret(const quint8 val)      { m_firstFret = val; emit firstFretChanged(); }
