...
{
    Q_OBJECT
    Q_PROPERTY(uint            number READ number WRITE number NOTIFY numberChanged)
    Q_PROPERTY(int             offset READ offset WRITE offset NOTIFY offsetChanged)
    Q_PROPERTY(int             channelId READ channelId WRITE channelId NOTIFY channelIdChanged)
...
public:
    ...
    static const int MAX_OFFSET = 24;
    static const int MIN_OFFSET = -24;

    // ---API--- //
    inline uint                   number()    const { return m_number; }
    inline int                    offset()    const { return m_offset; }
    inline int                    channelId() const { return m_channelId; }
    ...
    inline void number(const uint &val)              { m_number = val; emit numberChanged(); }
    inline void offset(const int &val)               { m_offset = val; emit offsetChanged(); }
    inline void channelId(const int &val)            { m_channelId = val; emit channelIdChanged(); }
    ...
    inline uint            measuresCount()                          const { return m_measures.count(); }
    inline const CMeasure& measure(const uint index)                const { return m_measures[index]; }
    inline void            measure(const uint index, const CMeasure &val) { m_measures[index] = val; m_measures[index].track(this); emit measuresChanged(); }
    inline void            measureAdd(const CMeasure &val)                { m_measures.append(val); m_measures.last().track(this); emit measuresChanged(); }
    inline void            measureRemove(const uint index)                { m_measures.removeAt(index); emit measuresChanged(); }
    ...
    inline const CString& string(const uint index)               const { return m_strings[index-1]; }
    inline void           string(const uint index, const CString &val) { m_strings[index-1] = val; emit stringsChanged(); }
    inline void           stringAdd(const CString &val)                { m_strings.append(val); emit stringsChanged(); }
    inline void           stringRemove(const uint index)               { m_strings.removeAt(index); emit stringsChanged(); }
