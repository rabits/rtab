    ...
    : public QObject
{
    ...
    Q_PROPERTY(int          index READ index WRITE index NOTIFY indexChanged)
    Q_PROPERTY(int          direction READ direction WRITE direction NOTIFY directionChanged)
...
public:
    explicit CVoice(const uint index, QObject *parent = 0);
    ...
    inline void index(const int val)           { m_index = val; emit indexChanged(); }
    inline void direction(const int val)       { m_direction = val; emit directionChanged(); }
    inline void empty(const bool val)          { m_empty = val; emit emptyChanged(); }
    inline void beat(CBeat *val)               { p_beat = val; emit beatChanged(); }

    inline uint          notesCount()                       const { return m_notes.count(); }
    inline const CNote&  note(const uint index)             const { return m_notes[index]; }
    inline void          note(const uint index, const CNote &val) { m_notes[index] = val; m_notes[index].voice(this); empty(false); emit notesChanged(); }
    inline void          noteAdd(const CNote &val)                { m_notes.append(val); m_notes.last().voice(this); empty(false); emit notesChanged(); }
    inline void          noteRemove(const uint index)             { m_notes.removeAt(index); emit notesChanged(); }
